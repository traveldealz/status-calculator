...
export default class extends BaseComponent {
  ...
  connectedCallback() {
    ...
    var selectedStatus = this.hasAttribute("status") ? this.getAttribute("status") : "Star Alliance Gold";
    var eid = document.getElementById("statusselector");

    for (var i = 0; i < eid.options.length; ++i) {
      if (eid.options[i].text === selectedStatus) eid.options[i].selected = true;
    }

    this.$whitelist = this.hasAttribute("whitelist") ? this.getAttribute("whitelist").split(",") : [];
    this.$programs = [];
  ...
  display({
    ...
    Object.keys(totals).map(id => {
      ...
      if (!program || this.$whitelist.length !== 0 && !this.$whitelist.includes(program.code)) {
        return [];
      }
