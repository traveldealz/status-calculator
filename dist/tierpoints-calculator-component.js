...
export default class extends BaseComponent {
  ...
  async getPrograms() {
    ...
    if (this.$alliance.length > 3) {
      response = await fetch("https://mileage.travel-dealz.eu/api/airline_programs?include=airlines&filter[has_qualification]=true&filter[alliance]=" + this.$alliance).then(response => response.json()).then(data => data.reduce((programs, item) => {
        programs[item.code] = item;
        return programs;
      }, {})).then(programs => this.$programs = programs);
    } else {
      response = await fetch("https://mileage.travel-dealz.eu/api/airline_programs?include=airlines&filter[has_qualification]=true&filter[code]=" + this.$alliance).then(response => response.json()).then(data => data.reduce((programs, item) => {
        programs[item.code] = item;
        return programs;
      }, {})).then(programs => this.$programs = programs);
    }
  ...
  async buildOptions() {
    ...
    let change = this;
    this.querySelector('[name="program"]').addEventListener("change", function (e) {
    ...
    this.querySelector('[name="status"]').addEventListener("change", event => {
      this.update_table();
    });
  ...
  connectedCallback() {
    ...
    this.$alliance = this.hasAttribute("alliance") ? this.getAttribute("alliance") : "";
    this.buildOptions();
  }
  /*
  ...
  display({
    ...
    this.rm_label = this.progs[this.$program.value].translations[this.$locale].rdm;
    this.qm_label = this.progs[this.$program.value].translations[this.$locale].qm;
    ...
    this.$segments.forEach((segment, index) => {
      let earning = data[index].value.totals.find(item => item.id == this.$program.value);
      console.log(earning);
