export default {
  ...
  calculateSegments: ({
    segments,
    program
  }) => segments.filter(segment => program.airlines.map(x => x.iatacode).includes(segment.carrier)).length,
  countMiles: ({
    data,
    program,
    segments
  ...
  calculateLHMSegements: ({
    segments
  }) => segments.filter(segment => ["EN", "OS", "SN", "OU", "EW", "LO", "LH", "LG", "LX"].includes(segment.carrier)).length,
  calculateLHMExecutivebonus: ({
    data
  }) => {
  ...
  countSVSegments: ({
    ...
    let mileage = itinerary.value?.totals?.find(item => program === item.id);
  ...
  calculateTGSegments: ({
    segments
  }) => segments.filter(segment => ["TG"].includes(segment.carrier)).filter(segment => !["G", "V", "W"].includes(segment.bookingClass)).length,
  calculateUXMiles: ({
    data,
    program,
    segments,
    ...
    if (segments[acc[1]].carrier && segments[acc[1]].ticketer) {
      if ([program].includes(segments[acc[1]].carrier) || [program].includes(segments[acc[1]].ticketer)) {
        let multiplier = 5;
