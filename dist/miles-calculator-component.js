...
import template from "./templates/miles-calculator.js";
export default class extends BaseComponent {
  ...
  calculate() {
    this.$programs = [];
    fetch("https://mileage.travel-dealz.eu/api/airline_programs").then(response => response.json()).then(data => data.reduce((programs, item) => {
  ...
  display({
    ...
    Object.keys(totals).map(id => {
      ...
      if (program.translations) {
        ...
        program.translations[this.$locale].qm_short ? build.qmname = program.translations[this.$locale].qm_short : build.qmname = program.translations[this.$locale].qm;
        program.translations[this.$locale].name ? build.programname = program.translations[this.$locale].name : build.programname = program.name;
      } else {
      ...
      return build;
    }).flat().filter(build => 1 < build.collected).flat().sort((a, b) => b.collected - a.collected).forEach(item => {
